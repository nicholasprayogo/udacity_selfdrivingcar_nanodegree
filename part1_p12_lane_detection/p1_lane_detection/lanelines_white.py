...
#for i in range(50, max_line_gap, 5):
    ...
    # print("vertices left",vertices_left)
    # print("vertices right",vertices_right)
    ...
    # take the average of lines on left and lines on right to reduce noise
    if len(leftlines)!=0:
        left_line = np.mean(leftlines, axis=0)
        left_line = left_line.astype(int)

        # code fix for extrapolation:
...
def main():
    ...
    while cap.isOpened():
        ...
        img = canny(gaussian_blur(grayscale(imgOriginal), 5), 50, 150)
        masked = roi(img, vertices)
        line_img = hough_lines(masked, imgOriginal, rho, theta, threshold, min_line_len, max_line_gap,vertices)
